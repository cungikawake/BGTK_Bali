...
<table width="100%" style="font-family: 'timesnewromanxx'; font-size: 16px; margin-bottom: 10px;" cellpadding="0">
	<tbody>
		...
		<tr>
			<td>Hari, Tanggal</td>
			<td>: <?php print $this->utility->formatRangeDay($tanggal[0], end($tanggal)); ?>, <?php print $this->utility->formatRangeDate($tanggal[0], end($tanggal)); ?></td>
		</tr>
	...
	<thead>
		...
		?>
				<tr>
					...
					<th class="center middle bold last-col" colspan="<?php print count($tanggal); ?>">Tanda Tangan</th>
				</tr>
				<tr>
					<?php
						$i = 1;
						$countDay = count($tanggal);
...
<table width="100%" style="font-family: 'timesnewromanxx'; font-size: 16px; vertical-align: top;">
	<tbody>
		<tr>
			<td>&nbsp;</td>
			<td><?php if (empty($kegiatan["kab_tempat_kegiatan"]) || $kegiatan["tipe_kegiatan"] == "Daring") {$tempat = $_ENV["DEFAULT_KABUPATEN"]; } else { $tempat = $kegiatan["kab_tempat_kegiatan"]; } print $tempat; ?>, <?php print $this->utility->formatDateIndo(end($tanggal)); ?></td>
		</tr>
		<tr>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
