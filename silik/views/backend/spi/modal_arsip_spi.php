<div class="modal-dialog modal-dialog-centered" role="document">
	<div class="modal-content">
		<div class="modal-header">
			...
			<h5 class="modal-title">APPROVAL LAPORAN [<?php print isset($kode) ? $kode : ""; ?>]</h5>
		</div>
		...
		<?php
			...
			if ($status != "Divalidasi SPI" && $status != "Ditolak SPI") {
				$disabled = 'disabled="disabled"';
			}
		?>
		
		<form action="/admin/spi/save_arsip" method="post" class="form-submit-spi" autocomplete="off">
				...
				<div class="card mb-0">
					...
					<div class="card-body">
											...
											?>
													<tr>
														...
														<td class="text-center" style="font-size:18px;">
															...
															?>
															<input type="hidden" name="uraian[<?php print $keyItem; ?>][valid]" value="0" />
															<input type="checkbox" name="uraian[<?php print $keyItem; ?>][valid]" value="1" <?php print $checked; ?> class="valid-check" />
														</td>
													</tr>
											<?php
													}
						...
						<div class="row">
							<div class="col-md-12">
								<div class="form-group">
									<label>Keterangan Setuju/Tolak</label>
									<textarea <?php print $disabled; ?> name="keterangan_spi" class="form-control" rows="5"><?php print isset($keterangan_spi) ? $keterangan_spi : ""; ?></textarea>
								</div>
							</div>
						</div>
					</div>
			...
			<div class="modal-footer">
				<?php
					if (empty($disabled)) {
				?>
						<button type="submit" class="btn btn-info btn-modal-form-submit" name="approval" value="setuju">Setuju</button>
						<button type="submit" class="btn btn-danger btn-modal-form-submit" name="approval" value="tolak">Tolak</button>
				<?php
					}
				?>
...
<script>
	$(document).ready(function () {
		...
		$(document).on("submit","form.form-submit-spi",function (e) {
			...
			if (approval == "setuju") {
				// check checked valid
				$('.table-uraian-arsip [type="checkbox"]').each(function () {
				...
				if (!ext_submit) {
					Swal.fire({
						icon: 'error',
						title: 'Oops...',
						text: "Ada item yang belum valid"
					});
				}
			}
			else if (approval == "tolak") {
				if($('[name="keterangan_spi"]').val() == "") {
					Swal.fire({
						icon: 'error',
						title: 'Oops...',
