<div class="modal-dialog modal-dialog-centered" role="document">
	<div class="modal-content">
		...
		<form action="/admin/spi/terima_laporan_kegiatan_kepala" method="post" class="form-submit" autocomplete="off">
				...
				success: function(obj){
					...
					if ('arsip' in obj && 'jenis_berkas' in obj.arsip && (obj.arsip.jenis_berkas == 'Laporan Keuangan dan BMN' || obj.arsip.jenis_berkas == 'Laporan Kegiatan')) {
						if ('nama' in obj.kegiatan) {
							...
							//$("#tgl-mulai-kegiatan").val(obj.kegiatan.tgl_mulai_kegiatan);
							//$("#tgl-selesai-kegiatan").val(obj.kegiatan.tgl_selesai_kegiatan);
						}
						else {
							$(".kegiatan-id").val("");
							$("#nama-kegiatan").val(obj.arsip.nama);
							//$("#tgl-mulai-kegiatan").val("");
							//$("#tgl-selesai-kegiatan").val("");
						}
						...
						if ('status' in obj.arsip && obj.arsip.status == "Divalidasi Kepala") {
							html = '<div class="alert alert-danger mt-3">Laporan ini sudah diterima pada tgl <strong>'+obj.arsip.tgl_laporan_diterima_kepala+'</strong> oleh <strong>'+obj.petugas_terima_kepala.nama+'</strong></div>';
							$('.arsip-msg').html(html);
							...
							html = '<div class="alert alert-danger mt-3">Laporan ini belum disetujui SPI</div>';
							$('.arsip-msg').html(html);
							$('.btn-modal-form-submit').attr("disabled", "disabled");
						}

						else if ('status' in obj.arsip && (obj.arsip.status != "Disetujui SPI" && obj.arsip.status != "Divalidasi Kepala" && obj.arsip.status != "Ditolak Kepala")) {
							html = '<div class="alert alert-danger mt-3">Laporan ini sudah disetujui pada tgl <strong>'+obj.arsip.tgl_laporan_disetujui_kepala+'</strong> oleh <strong>'+obj.petugas_setuju_kepala.nama+'</strong></div>';
							$('.arsip-msg').html(html);
							$('.btn-modal-form-submit').attr("disabled", "disabled");
						}
