...
<script type="text/javascript">
	...
	Biodata.init = function () {
		...
		$(document).on("submit","form.form-submit-registrasi2",function (e) {
			...
			if ($("#checkbox-konfirmasi_paket").length) {
				if (!$('#checkbox-konfirmasi_paket').is(':checked')) {
					Swal.fire(
						'Perhatian!',
						'Mohon konfirmasi akan hadir pada kegiatan ini',
						'warning'
					).then((result) => {
						$('html, body').animate({
							scrollTop: $('#label-checkbox-konfirmasi_paket').offset().top-200
						}, 500, function(){$('#label-checkbox-konfirmasi_paket').focus();});
					});
	...
	Biodata.findByNIK = function (nik) {
		...
		$.ajax({
			...
			success: function(obj){
				...
				$('[name="ptkp"]').prop("disabled",false).val(obj.ptkp).trigger("change");
				...
				$('[name="npsn"]').removeAttr("disabled").val(obj.npsn);
