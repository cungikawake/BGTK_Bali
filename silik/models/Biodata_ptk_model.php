...
class Biodata_ptk_model extends CI_Model{
	...
	public function countBiodata () {
		$count = $this->db->count_all('biodata_ptk');
	...
	public function save ($data, $id = 0) {
		if (empty($id)) {
			...
			$this->db->insert("biodata_ptk", $data);
			...
			$this->db->where("id", $id);
			$this->db->update("biodata_ptk", $data);
		}
	...
	public function updateDataBank ($data) {
		$data['diubah_tgl'] = date("Y-m-d H:i:s");
		...
		$data["valid_rekening"] = 1;

		$this->db->like('nama_bank', $data["nama_bank"]);
